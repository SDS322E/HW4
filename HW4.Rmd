...
First load the `tidyverse` package so that we can have access to the `dplyr` data transformation functions. We also want to look at the `penguins` dataset, which is inside the `palmerpenguins` package:
...
Using a `dplyr` function, keep all the rows/observations in the `penguins` dataset from the year 2007 and save the result as a new object called `penguins_2007`. Compare the number of observations/rows in the original `penguins` dataset with your new `penguins_2007` dataset.
